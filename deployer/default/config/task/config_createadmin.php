...
task('config:createadmin', function () {
    ...
    if (empty($username)) {
        throw new Exception('Username has to be defined (env DEP_CONFIG_CREATEADMIN_USERNAME or answer a question).',
            1560175164);
    }
    ...
    if (get('is_argument_host_the_same_as_local_host')) {
        try {
            ...
            output()->writeln('<info>Admin for instance "' . get('local_host') . '" has been created.</info>');
        } catch (\Exception $e) {
            output()->writeln('<error>Admin for instance "' . get('local_host') . '" could not be created.</error>');
        }
    } else {
        try {
            $activeDir = get('deploy_path') . (test('[ -e {{deploy_path}}/release ]') ? '/release' : '/current');
            $verbosity = (new ConsoleUtility())->getVerbosityAsParameter();
            run('cd ' . $activeDir . ' && {{bin/php}} {{bin/deployer}} config:createadmin ' . get('argument_host') . ' --username ' . escapeshellarg($username) . ' --password ' . escapeshellarg($password) . ' ' . $verbosity);
        } catch (\Exception $e) {
            output()->writeln('<error>Admin for instance "' . get('argument_host') . '" could not be created.</error>');
            throw $e;
        }
    }
